...
<div class="flex-1 p-8 space-y-8 max-w-7xl mx-auto w-full relative">
    <div class="flex items-center gap-4">
        ...
        <div class="space-y-1">
            <h1 class="text-3xl font-extrabold tracking-tight text-on-surface">Edit Berita</h1>
            <p class="text-on-surface-variant font-medium">Ubah konten artikel atau status publikasi.</p>
        </div>
    </div>
    
    <?= renderFlash() ?>
    <form action="index.php?action=berita_update" method="POST" enctype="multipart/form-data" class="space-y-8 pb-32">
    <?= csrf_field() ?>
            ...
            <div class="lg:col-span-2 space-y-8">
                <div class="bg-surface-container-lowest p-8 rounded-2xl shadow-sm space-y-6">
                        ...
                        <div class="relative">
                            <select name="ukm_id" required class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-primary/20 appearance-none cursor-pointer">
                                ...
                                <?php foreach ($ukmList as $u): ?>
                                    <option value="<?= $u['id'] ?>" <?= ($b['ukm_id'] == $u['id']) ? 'selected' : '' ?>><?= htmlspecialchars($u['nama']) ?></option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <input name="judul" class="w-full text-2xl font-bold bg-transparent border-none border-b-2 border-surface-container-high focus:border-primary focus:ring-0 px-0 pb-2 transition-all placeholder:text-surface-dim" placeholder="Masukkan judul yang menarik..." type="text" value="<?= htmlspecialchars($b['judul'] ?? '') ?>" required/>
                    </div>
                    <div class="space-y-2 pt-4">
                        ...
                        <div class="bg-surface-container-low rounded-xl overflow-hidden border border-transparent focus-within:border-primary/20 transition-all">
                            ...
                            <div id="rich-editor" contenteditable="true" class="w-full min-h-[350px] bg-transparent border-none focus:outline-none p-4 text-on-surface leading-relaxed format-content prose prose-blue max-w-none"><?= $b['konten'] ?? '' ?></div>
                            <textarea id="hidden-konten" name="konten" class="hidden"><?= htmlspecialchars($b['konten'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-surface-container-lowest p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div class="relative aspect-video rounded-xl border-2 border-dashed border-outline-variant bg-surface-container-low flex flex-col items-center justify-center gap-3 p-4 text-center group cursor-pointer hover:bg-surface-container-high transition-all" onclick="document.getElementById('gambar-input').click()">
                        ...
                        <?php if (!empty($b['gambar_path'])): ?>
                            <img src="<?= htmlspecialchars($b['gambar_path']) ?>" class="absolute inset-0 w-full h-full object-cover rounded-xl" alt="Current">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <select name="kategori" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-primary/20 appearance-none cursor-pointer">
                                <option disabled value="" <?= empty($kat) ? 'selected' : '' ?>>Pilih Kategori...</option>
                                ...
                                <option value="Kegiatan" <?= $kat === 'Kegiatan' ? 'selected' : '' ?>>Kegiatan</option>
                                <option value="Informasi" <?= $kat === 'Informasi' ? 'selected' : '' ?>>Informasi</option>
                            </select>
                            <span class="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-on-surface-variant">expand_more</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 pt-2">
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input name="published" class="sr-only peer" type="checkbox" value="1" <?= ($b['status'] ?? '') === 'published' ? 'checked' : '' ?>/>
        ...
        <div class="fixed bottom-0 right-0 left-[260px] bg-white/60 backdrop-blur-md border-t border-surface-container-high px-8 py-4 z-40 flex items-center justify-end gap-4 shadow-[0_-10px_40px_rgba(25,28,30,0.02)]">
            <a href="index.php?page=berita" class="px-6 py-2.5 rounded-xl text-sm font-bold text-on-surface-variant hover:bg-surface-container-high transition-all">
                Batal
            </a>
            <button class="flex items-center gap-2 px-8 py-2.5 rounded-xl text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all active:scale-95" type="submit">
...
document.getElementById('gambar-input').addEventListener('change', function(e) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            const oldImg = document.querySelector('.cursor-pointer > img.absolute');
